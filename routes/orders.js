...
const { sendPaymentURL, requestTabbyPaymentURL } = require("../controller/paymentController");
const { sendMail } = require("../controller/mailController");
const { TailorRequest, DeliveryAddress } = require("../models");
...
router.post("/tabby_response", (req, res) => {
  ...
  console.log('====================================');
  axios.get(`https://api.tabby.ai/api/v1/payments/${req.body.query}`,{
    headers: {
    ...
    console.log('====================================');
    if (response.data.status && response.data.status == "AUTHORIZED") {
      models.Order.update(
        {
          status: 'pickable',
...
router.post("/payment_response", (req, res) => {
  ...
  console.log(req.body);
  console.log('====================================');
  axios.get(`https://api.tap.company/v2/charges/${req.body.query}`,{
    headers: {
...
router.post("/tailor_order_update",
(req, res) => {
  console.log('====================================');
  console.log(req.body);
  console.log('====================================');
  TailorRequest.update({status : req.body.status},{where : {id : req.body.id}}).then(response => {
    return res.status(200).json({
      status: true,
...
router.get("/tailor_orders",
(req, res) => {
  TailorRequest.findAll({
...
router.post(
  ...
  (req, res) => {
    ...
    const allCartCustomizeRelationshipsQuery = models.CartCustomizeRelationship.findAll();

    requests.push(cart);
    requests.push(generalOptionFetch);
    requests.push(allCartCustomizeRelationshipsQuery);
    if(req.body.deliveryMethod == "homeDelivery"){
      requests.push(DeliveryAddress.create({user_id: req.user.id, ...req.body.address}))
    }
      ...
      if (cart.CartProductRelationships.length > 0) {
        return sequelize
              ...
              .createOrder(
                {
                  ...
                  address:
                    req.body.address && req.body.address.region && req.body.address.region != ""
                  ...
                  delivery_address_id:
                    req.body.deliveryMethod == "homeDelivery" ? results[3].id : null,
                },
                { transaction: t }
              )
              .then((order) => {
                ...
                cart.CartProductRelationships.map((item) => {
                  ...
                  var newPromise = OrderProductRelationship.create(
                    ...
                    let productCartCustomizations = allCartCustomizeRelationships.filter(
                      (i) => {
                ...
                var sizeMan = !req.body.sizeManFlag
                  ...
                  : generalOptionsList.find((opt) => opt.key === "man_price")
                      .value;
                console.log(`1111111111111111 total is ${total}`);
                console.log(`11111111111 request is ${req.body.coupon_code}`);
                console.log(`11111111111 request is ${req.body.MDNDiscount}`);
                console.log(new Date());
                console.log(new Date("4/2/2021"));
                console.log(new Date("4/3/2021"));
                console.log((new Date() >= new Date("4/2/2021")) && (new Date() <= new Date("4/7/2021")));
                // check if there a discount coupon
                if(req.body.coupon_code && req.body.coupon_code === "MDN" && (new Date() >= new Date("4/6/2021")) && (new Date() <= new Date("4/7/2021")) && req.body.MDNDiscount){

                  order.update(
                    {
                      coupon_discount: req.body.MDNDiscount,
                      coupon_code: req.body.coupon_code
                        ? req.body.coupon_code
                        : null,
                      expected_total:
                        total *
                          (1 - (userDiscount + partnerDiscount) /100) - req.body.MDNDiscount +
                        parseFloat(
                          req.body.deliveryMethod == "homeDelivery"
                            ? deliveryCost
                            : 0
                ...
                }else if (req.body.coupon_code && req.body.coupon_code != null) {
                  coupon = GeneralCoupon.findOne({
                    ...
                    console.log("discount_coupon is" + discount_coupon);
                ...
                if (req.body.paymentMethod == "creditCard") {
                  paymentData = {
                    amount:
                      total *
                        (1 -
                          (userDiscount + partnerDiscount + couponDisc) / 100) +
                      parseFloat(
                        req.body.deliveryMethod == "homeDelivery"
                          ? deliveryCost
                          : 0
                ...
                return Promise.all([
                  ...
                  sendMessage(
                    [req.user.mobile],
                    req.body.language === 1
                      ? `Your order ${order.id} is now confirmed`
                      : `تم تأكيد رقم ${order.id}`,
                    "iThoob",
                    "orderStatus"
                  ),
                  sendMail(
          ...
          .then((result) => {
            ...
            }else if(req.body.paymentMethod == "tabbyPayment"){
              requestTabbyPaymentURL(result[result.length -1].newOrder, req.user).then((tabbyResponse) => {
                console.log("tabbyResponse tabbyResponse tabbyResponse");
                console.log(tabbyResponse);
                responseUrl = tabbyResponse.data.configuration.available_products.installments[0].web_url;
                console.log(responseUrl);
                res.status(200).json({
                  status: true,
                  orderNo: result[0].orderNo,
                      ...
                      expectTotal:
                        ...
                        order.OrderProductRelationships.find((item) => {
                          ...
                          ? String(
                              ...
                              (
                                order.total *
                                  (1 - (userDiscount + partnerDiscount  + (order.coupon_code !== "MDN" ? order.coupon_discount : 0)) / 100) -(order.coupon_code === "MDN" ? order.coupon_discount : 0) +
                                parseFloat(order.delivery_cost) +
                                parseFloat(order.size_man)
                              ).toFixed(2)
                            )
                          : order.coupon_code !== "MDN" ? 
                          String(
                              // There's NO sizeMan cost!
                              (
                                order.total *
                                  (1 - (userDiscount + partnerDiscount + order.coupon_discount) / 100) + parseFloat(order.delivery_cost)
                              ).toFixed(2)
                            ) : String(
                              // There's NO sizeMan cost!
                              (
